...
public class LazyListTest {
  ...
  public void testLaziness() throws Exception {
    ...
    IList<String> ucaseList = longerList.map(String::toUpperCase); // because of laziness, this shouldn't have gone beyond Charlie

    // okay, now we're going to read the elements from the lazy list in series
    assertEquals("ALICE", ucaseList.head());
    verify(supplier, never()).get();
    assertEquals("BOB", ucaseList.tail().head());
    verify(supplier, never()).get();
    assertEquals("CHARLIE", ucaseList.tail().tail().head());
    verify(supplier, never()).get();
    assertEquals("DOROTHY", ucaseList.tail().tail().tail().head());
    verify(supplier, never()).get();
    assertEquals("EVE", ucaseList.tail().tail().tail().tail().head());
