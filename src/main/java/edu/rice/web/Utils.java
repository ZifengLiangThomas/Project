...
public interface Utils {
   ...
   * it might not.
   */
  static void launchBrowser(@NotNull String url) {
  ...
  @NotNull
  @Contract(pure = true)
  static ContainerTag muicssHeader(@NotNull String pageTitle, @NotNull String jsLibrary, @NotNull String cssFile) {
    return head().with(
        title(pageTitle),
        ...
        script().withType("text/javascript").withSrc("//cdnjs.cloudflare.com/ajax/libs/react/0.13.2/react.min.js"),
        script().withType("text/javascript").withSrc("//cdnjs.cloudflare.com/ajax/libs/react/0.13.2/JSXTransformer.js"),
        script().withType("text/javascript").withSrc("/mui-0.6.0/react/mui-react.min.js"),
        script().withType("text/javascript").withSrc("/snack-min.js"),
  ...
  @NotNull
  @Contract(pure = true)
  static ContainerTag muicssCommandLineBody(@NotNull String pageTitle) {
    return body().with(
        header()
            .withId("header").with(
        ...
        div().withId("footer").with(Utils.textEntryForm("commandLine", "type your commands here")));
  }

  /**
   * j2html helper used for making text entry forms.
   */
  @NotNull
  @Contract(pure = true)
  static ContainerTag textEntryForm(@NotNull String id, @NotNull String placeHolder) {
    return form().with(
        div().withClass("mui-textfield").with(input()
            .withId(id)
            .withType("text")
  ...
  @NotNull
  @Contract(pure = true)
  static Value requestToJson(@NotNull Request request) {
    return jobject(
   ...
   */
  static void logSparkRequest(@NotNull String tag, @NotNull Request request) {
   ...
   */
  static void jsonSparkExceptionHandler(@NotNull String tag) {
    exception(Exception.class, (e, request, response) -> {
      ...
      // use for this, but it might be helpful if you're looking in a browser debuger.
